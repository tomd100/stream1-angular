...
angular.module("RouteControllers", [])
    ...
    .controller("TodoController", function($scope, $location, TodoAPIService, store){
        ...
        $scope.deleteTodo=function(id){
            TodoAPIService.deleteTodo(URL + "todo/" + id, $scope.username, $scope.authToken)
                .then(function(results){
                    console.log(results);
                    $scope.refreshTodos();
                }).catch(function(err){
    ...
    .controller("editTodoController", function($scope, $location, $routeParams, TodoAPIService, store){
        ...
        TodoAPIService.getTodos(URL + "todo/" + id, $scope.username, $scope.authToken)
            .then(function(results){
        ...
        $scope.submitForm = function() {
            if ($scope.todoForm.$valid) {
                ...
                TodoAPIService.editTodo(URL + "todo/" + id, $scope.todo, store.get('authToken'))
                    .then(function(results) {
                        $location.path("/todo");
                    }).catch(function(err) {
                        console.log(err);
